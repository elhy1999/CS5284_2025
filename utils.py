...
def reindex_W_with_classes(
    W: scipy.sparse.csr_matrix, C: np.ndarray
...
def graph_laplacian(
    W: scipy.sparse.csr_matrix, normalized: bool = True
...
def graph_gradient(W: scipy.sparse.csr_matrix) -> scipy.sparse.csr_matrix:
    """
    Compute the graph gradient operator.

    Parameters:
        W (scipy.sparse.csr_matrix): Adjacency matrix of the graph (n x n).

    Returns:
        scipy.sparse.csr_matrix: Graph gradient operator (m x n), where m is the number of edges.
    """
    W = W.todense()
...
def sortEVD(lamb: np.ndarray, U: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
    Sort eigenvalues and eigenvectors in ascending order of eigenvalues.

    Parameters:
        lamb (np.ndarray): Array of eigenvalues.
        U (np.ndarray): Matrix of eigenvectors.
...
def compute_ncut(
    W: scipy.sparse.csr_matrix, Cgt: np.ndarray, R: int
...
def construct_knn_graph(X: np.ndarray, k: int, dist: str) -> scipy.sparse.csr_matrix:
    n = X.shape[0]
    ...
    if dist == "euclidean":
        ...
        ## In this method, the weights are computed using a global scale parameter sigma2, which is the squared mean
        ## distance to the k-th nearest neighbors over all nodes.
    ...
    if dist == "euclidean_zelnik_perona":
        ...
        # Compute Weight matrix
        ## In this method, the weights are computed using a local scale parameter for each node, based on the distance
        ## to its k-th nearest neighbor.
        sigma = D[:, -1]
        D = np.divide(
            (D**2).reshape(n * k),
            np.multiply(sigma[np.arange(0, n).repeat(k)], sigma[idx.reshape(n * k)]),
        )
        W = np.exp(-D)
...
def sortPCA(lamb: np.ndarray, U: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def plant_seeds(C: np.ndarray, n: int, R: int, seeds_per_class: int) -> np.ndarray:
    """
    Plant seeds for incremental clustering.

    Parameters:
        C (np.ndarray): Cluster assignments for each data point.
        n (int): Number of data points.
        R (int): Number of clusters.
        seeds_per_class (int): Number of seeds to plant per cluster.

    Returns:
        np.ndarray: Indicator matrix for seeds (n x R).
    """
    # Init
    F = np.zeros((n, R))
    Bal = np.zeros(R)
...
def compute_SVM(
    Xtrain: np.ndarray,
    Cgt_train: np.ndarray,
    l_train: np.ndarray,
    type_svm: str,
    param_svm: Optional[List[float]] = None,
    Xtest: Optional[np.ndarray] = None,
    Cgt_test: Optional[np.ndarray] = None,
    plot: Optional[List[int]] = None,
    type_graph: Optional[str] = None,
    param_graph: Optional[List[float]] = None,
...
def shrink(x: np.ndarray, mu: float) -> np.ndarray:
    """
    Apply the soft shrinkage operator to the input array.

...
def interactive_vis_graph(G_nx, countries, node_labels):
    """
    Creates an interactive Plotly graph where nodes are colored based on the node_labels.

    Args:
        G_nx: The NetworkX graph.
        countries: List of country names (ordered by the node index in the graph).
        node_labels: List of labels for each node, where nodes with the same label share the same color.
    """
    # Get positions for the nodes in a 2D layout
    ...
    for edge in G_nx.edges():
        ...
        edge_x.append(x0)
        edge_x.append(x1)
        edge_x.append(None)  # Separate the edges
        edge_y.append(y0)
        edge_y.append(y1)
        edge_y.append(None)
    ...
    # Extract node coordinates and labels for Plotly
    node_x = []
    node_y = []
    ...
    # Create Plotly node trace, coloring nodes based on labels
    node_trace = go.Scatter(
        ...
        marker=dict(
            showscale=True,
            colorscale='Viridis',  # You can change the colorscale to fit your needs
            size=10,
            color=node_labels,  # Use node_labels for color
            colorbar=dict(
                thickness=15,
                title='Node Labels',
                xanchor='left',
            ...
            line_width=2),
        text=countries)  # Country names as hover text

    # Create the Plotly figure
    fig = go.Figure(data=[edge_trace, node_trace],
                    layout=go.Layout(
                        title='Interactive KNN Graph of Countries',
                        titlefont_size=16,
                        showlegend=False,
                        hovermode='closest',
...
def print_neighboring_countries(G, countries, target_country):
    """
    Prints the neighboring countries for a given country in the graph.

    Args:
        G: The NetworkX graph.
        countries: List of country names (ordered by the node index in the graph).
        target_country: The country for which to find neighboring countries.
    """
    # Find the index of the target country in the countries list
    if target_country not in countries:
        print(f"Country '{target_country}' not found in the dataset.")
        return

    target_node = countries.tolist().index(target_country)

    # Get neighboring nodes of the target node
    neighbors = list(G.neighbors(target_node))

    # Print the neighboring countries
    if neighbors:
        print(f"Neighboring countries of {target_country}:")
        for neighbor in neighbors:
            print(f"- {countries[neighbor]}")
    else:
        print(f"{target_country} has no neighboring countries in the graph.")


def get_same_label_countries(countries, node_labels, queried_country):
    """
    Given a list of countries, a list of node labels, and a specific country name,
    this function returns other countries that share the same label as the queried country.

    Args:
        countries: List of country names.
        node_labels: List of labels corresponding to each country.
        queried_country: The country for which you want to find other countries with the same label.

    Returns:
        List of countries with the same label as the queried country.
    """
    # Ensure the queried country is in the list of countries
    if queried_country not in countries:
        return f"Country '{queried_country}' not found in the list of countries."

    # Get the index and label of the queried country
    queried_index = countries.tolist().index(queried_country)
    queried_label = node_labels[queried_index]

    # Find all countries with the same label as the queried country
    same_label_countries = [countries[i] for i, label in enumerate(node_labels) if label == queried_label and i != queried_index]
    print(f'Countries in the same cluster as {queried_country}: {same_label_countries}')
